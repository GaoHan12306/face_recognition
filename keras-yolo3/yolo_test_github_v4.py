# -*- coding: utf-8 -*-
"""
功能：keras-yolov3 进行批量帧测试 并 保存结果，输送含有人的帧（>0）到edge进行openpose进一步姿势识别
原有项目来源：https://github.com/qqwweee/keras-yolo3
修改：GH
"""
...
class YOLO(object):
    ...
    def detect_image(self, image):
        ...
        for i, c in reversed(list(enumerate(out_classes))):
            ...
            coords.append([top, bottom, left, right]) #ZBW add
...
def image():
    ...
    for filename in os.listdir(path):
        ...
        if (reason_number > 0):  # 传输 人数>0的图像给edge端
            client_zbw_v2.send_img(image_save_path, coords)
            # client_zbw_v2.send_coords()

            # # 显示图像 ----GH添加
            # test_img1 = cv2.imread(image_save_path)
            # cv2.namedWindow('', cv2.WINDOW_NORMAL)
            # cv2.imshow('', test_img1)
            # cv2.waitKey(1000)  # 等待键盘输入时间 0代表无限等待
            # cv2.destroyAllWindows()  # ----GH添加
...
if __name__ == '__main__':
    client_zbw_v2.send_name()  # 发送收货人姓名
    signal = input('无人机到达地点 给出指令>>>')  # 无人机到达地点 给出指令
    if signal == '1':
        t1 = time.time()  # 开始收货计时
        image()
